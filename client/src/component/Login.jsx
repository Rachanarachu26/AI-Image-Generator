...
const { setShowLogin, backendUrl, setToken, setUser } = context;
  ...
  return (
    ...
    right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30
    ...
    text-slate-500'>
      ...
      text-2xl text-neutral-700 font-medium'>{state}</h1>
      <p className='text-sm'> 
      ...
      my-4 cursor-pointer'>Forgot password?</p>    


      <button className='bg-blue-600 w-full text-white
      py-2 rounded-full'>{state === 'Login' ? 'Login' : 'cretae account'}</button>
     ...
     {state === 'Login' ?<p className='mt-5 text-center'>Don't have an account? 
      ...
      src={assets.cross_icon} alt=""  className='
